...
> 我们给 AI 编程智能体套上了五种意识形态，然后跑了一批真实的工程基准测试。
> **马克思主义赢了。** 凯恩斯主义把结果搞砸了。其他的大多有点用。

这最开始是个怪想法：如果智能体思考代码的方式——底层的世界观——真的有影响呢？结果发现确实有。而且影响比预期大。
...
马克思主义技能不是噱头。它把辩证唯物主义映射成了一套具体的排查流程——而且这个映射出乎意料地贴切。

**核心规则：你不能凭记忆回答。**

其他框架允许智能体从自己已知的知识出发推理。马克思主义方法规定了一件必须做的事：*在给出任何预测之前，先检查这个具体的仓库*。跑 `find`，跑 `grep`，读实际的 import 链。然后再回答。
...
实用主义技能的提升幅度几乎一样大，靠的是另一条具体规则：*优先选父类文件而不是子类文件*。简单、具体、有效。
...
一个被 50 个模块引用的工具文件，几乎不可能是某个具体 bug 的所在地。越「重要」的文件，越不可能是你要找的答案。凯恩斯主义每次都系统性地把智能体指向了错误的地方。

---

## 🛠️ 五种技能

每个技能都是一个 `SKILL.md`——一份结构化的方法论提示词，可以直接丢进任何智能体。
...
每个技能目录包含主 `SKILL.md` 和 `references/` 哲学参考文件夹。
...
- **不是政治表态。** 这些是工程方法论，只是恰好有意识形态的名字。
- **不是提示词魔法。** 提升来自具体的排查流程，不是氛围感。
- **不是放之四海而皆准。** 在事实召回（SimpleQA）和纯代码生成（SWE-bench）上，效果更小或方向相反。
...
- [HughYau/qiushi-skill](https://github.com/HughYau/qiushi-skill) —— 最初的启发：毛泽东方法论可以直接指导工程实践
